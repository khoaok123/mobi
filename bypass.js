// ==UserScript==
// @name         Bypass Yeumoney V2m - Hash Key
// @namespace    http://tampermonkey.net/
// @version      2.3
// @description  Bypass Yeumoney with offline hashed key
// @author       duongthekhoa
...
(function () {
  ...
  function simpleHash(str) {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      hash = hash & hash; // Giới hạn số
    }
    return Math.abs(hash).toString(36); // Chuyển sang base36 để ngắn gọn
  }

  // Hàm tạo key offline dựa trên hash ngày tháng
  ...
  (async () => {
    ...
    function _0x6749c7() {
      ...
      btn.click();
      setTimeout(() => {
        const link = document.querySelector("#lydo_doima > center > a:nth-child(2)");
        if (link) link.click();
        setTimeout(() => {
          const radio = document.querySelector("#lydo_doima > label:nth-child(8) > input[type=radio]");
    ...
    function _0x2873d5() {
      return new Promise((resolve, reject) => {
        ...
        const site = document.querySelector("p#TK1")?.textContent.trim().toLowerCase() || "";
        const img = document.querySelector("img#halt_nv") || document.querySelector("img#hinh_nv");
        ...
        xhr.onload = () => {
          if (xhr.status === 200) {
            ...
            const lines = data.ParsedResults[0].TextOverlay.Lines.filter(line =>
              line.LineText.match(/\b[a-zA-Z0-9-]+\.[a-zA-Z0-9-]+\b/) &&
              line.Words.some(word => word.Top < 170)
            ).map(line => line.LineText);
            let finalUrl = "";
            if (site === "188bet") finalUrl = "https://165.22.63.250/" + lines + "/";
            else if (site === "w88") finalUrl = "https://188.166.185.213/";
            else if (site === "bk8") finalUrl = "https://188.166.189.40/" + lines + "/";
            else if (site === "fb88") finalUrl = "https://fb88xn.com/" + lines + "/";
            else if (site === "m88") finalUrl = "https://bet88" + lines + "/";
            else if (site === "vn88") finalUrl = "https://vn88ce.com/" + lines + "/";
            else if (site === "v9bet") finalUrl = "https://188.166.224.89/" + lines + "/";
    ...
    function _0x2fa783(clk) {
      return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
          if (xhr.status === 200) {
            const match = xhr.responseText.match(/localStorage\.codexn\s*=\s*'([^']+)'/);
            if (match) {
    ...
    function _0x1b4021(codexn, url, traffic, clk) {
      return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
          if (xhr.status === 200) {
            const match = xhr.responseText.match(/sessionStorage\.setItem\("ymnclk", (\d+)\)/);
            if (match) {
    ...
    function _0x313435(code) {
      const action = document.querySelector("#gt-form")?.getAttribute("action") || "";
      const url = "https://yeumoney.com" + action;
      const form = new FormData();
      form.append("code", code);
      form.append("keyword", "");
      form.append("dieuhanh", document.querySelector('input[name="dieuhanh"]')?.value || "");
      form.append("pix", document.querySelector('input[name="pix"]')?.value || "");
      form.append("lvp", document.querySelector('input[name="lvp"]')?.value || "");
      form.append("ref", "$ref");
      form.append("trinhduyet", document.getElementById("trinhduyet")?.value || "");
      form.append("id_traffic", document.getElementById("id_donhang")?.value || "");
      ...
      GM_xmlhttpRequest({
        ...
        onerror: (err) => window.location.href = err.error.match(/https?:\/\/[^\s"]+/)
      });
    }
    ...
    function _0x3d4ed8(detectedUrl) {
      ...
      style.textContent = "@keyframes rainbowMove { 0% { background-position: 0% 50%; } 100% { background-position: 100% 50%; } }";
      ...
      urlLabel.style.cssText = "margin: 0; font-weight: bold; margin-bottom: 10px; font-size: 16px; color: darkred;";
      panel.appendChild(urlLabel);

      const urlInput = document.createElement("input");
      urlInput.placeholder = "Nếu để trống sẽ dùng URL nhận diện!";
      urlInput.style.cssText = "width: 100%; margin-bottom: 10px; padding: 8px; font-size: 14px;";
      panel.appendChild(urlInput);

      const detected = document.createElement("h4");
      detected.textContent = "URL nhận diện (OCR): " + detectedUrl;
      detected.style.cssText = "margin: 0; margin-bottom: 10px; font-size: 13px; color: brown;";
      panel.appendChild(detected);

      const options = document.createElement("div");
      options.style.cssText = "display: flex; align-items: center; margin-bottom: 10px; font-size: 14px; color: chocolate;";
      ...
      buttons.style.cssText = "display: flex; justify-content: space-between; font-size: 14px;";

      const bypassBtn = document.createElement("button");
      bypassBtn.textContent = "Bắt đầu Bypass";
      bypassBtn.style.cssText = "flex: 1; padding: 7px; background: #4CAF50; color: #fff; border: none; cursor: pointer; margin-right: 5px; border-radius: 5px;";
      ...
      changeBtn.style.cssText = "flex: 1; padding: 7px; background: #F44336; color: #fff; border: none; cursor: pointer; margin-right: 5px; border-radius: 5px;";
